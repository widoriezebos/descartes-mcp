 ...
 *
 * <p>
 * The scoring algorithm prioritizes threads that are most likely to be relevant
 ...
 * <li>Non-daemon application threads are prioritized over background
 * threads</li>
...
public class ThreadImportanceScorer {
  ...
  private static final Set<String> JVM_SYSTEM_THREADS = Set.of("Reference Handler", "Finalizer", "Signal Dispatcher",
      "Attach Listener", "Common-Cleaner", "Notification Thread", "DestroyJavaVM");

  // Patterns for recognizing JVM-internal threads
  private static final Pattern GC_THREAD_PATTERN = Pattern
      .compile("^(GC |G1 |ZGC |Shenandoah |VM |C[12] CompilerThread|Service Thread).*", Pattern.CASE_INSENSITIVE);

  // Patterns for recognizing thread pools
  private static final Pattern THREAD_POOL_PATTERN = Pattern.compile(".*(-pool-|-executor-|-worker-|-thread-).*",
  ...
  /**
   * Creates a scorer with default configuration (no app-specific package
   * bonuses).
   */
  ...
  /**
   * Creates a scorer with application-specific package patterns for bonus
   * scoring.
   *
   * @param appPackagePatterns Set of package prefixes that identify application
   *                           code (e.g., "com.mycompany", "com.bitsapplied")
   */
  ...
  public static boolean isJvmSystemThread(String threadName) {
    return JVM_SYSTEM_THREADS.contains(threadName) || GC_THREAD_PATTERN.matcher(threadName).matches();
  }
